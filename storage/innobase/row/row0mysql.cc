...
ATTRIBUTE_COLD
static dberr_t row_mysql_get_table_status(trx_t *trx, dict_table_t *table)
{
	...
	ins_mode_t	ins_mode)
{
	...
	if (!prebuilt->table->space) {
		...
		return row_mysql_get_table_status(trx, prebuilt->table);
...
row_update_for_mysql(row_prebuilt_t* prebuilt)
{
	...
	if (!table->is_readable()) {
		return row_mysql_get_table_status(trx, table);
	}
